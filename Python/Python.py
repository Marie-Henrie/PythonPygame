...
# Lataa muiden kissojen kuvat ja niihin liittyvät tekstit
level_cats = {
    1: [
...
# Funktio puhekuplien piirtämiseen
def draw_speech_bubble(text, position):
    bubble_width, bubble_height = 200, 100
    bubble_x, bubble_y = position[0] - bubble_width // 2, position[1] - bubble_height - 40  # Nosta ylös
    ...
    for word in words:
        if bubble_font.size(current_line + word)[0] <= bubble_width - 20:  # Jätä tilaa reunalle
            current_line += word + ' '
        else:
    ...
    for i, line in enumerate(lines):
        text_surface = bubble_font.render(line.strip(), True, BLACK)  # Poista ylimääräinen välilyönti
        text_rect = text_surface.get_rect(center=(bubble_x + bubble_width // 2, bubble_y + 20 + i * bubble_font.get_height()))
        window.blit(text_surface, text_rect)


# Funktio kissojen luomiseen satunnaisiin paikkoihin ilman, että ne päällekkäin pelaajan kissan kanssa
def create_cats(num_cats, level, player_rect):
    ...
    for _ in range(num_cats):
        while True:
            ...
            # Varmistetaan, ettei ole törmäystä pelaajan kissan kanssa
            if player_rect.colliderect(new_cat_rect):
                continue  # Siirry seuraavaan iteraatioon, jos on törmäys

            # Tarkistetaan myös ettei kissa törmää heti muihin luotuihin kissoihin
            collision = False
            for cat in cats:
...
def game_loop():
    ...
    speech_bubble_active = False  # Alustetaan puhekupla pois päältä
    ...
    while level <= max_level:
        ...
        # Luodaan kissoja, kunnes pelaajan kissa ei osu yhdenkään eläimen kanssa
        while True:
            other_cats = create_cats(num_cats, level, player_rect)  # Luo kissoja nykyiselle tasolle
            
            # Tarkistetaan onko pelaajan kissa päällekkäinen minkään eläimen kanssa
            collision = any(player_rect.colliderect(cat["rect"]) for cat in other_cats)
            if not collision:
                break  # Jos ei ole törmäyksiä, lopetetaan silmukka
        ...
        while running:
            ...
            # Rajoita pelaajan kissan liikettä ikkunan sisällä
            if player_rect.x < 0:
                player_rect.x = 0
            ...
            # Piirrä pelaajan kissa
            window.blit(player_cat_img, player_rect)

            # Tarkista törmäykset muiden kissojen kanssa
            for cat in other_cats:
                if not cat["visible"] and player_rect.colliderect(cat["rect"]):
                    cat["visible"] = True  # Tee kissa näkyväksi
                    score += 1  # Lisää pisteitä
                    speech_bubble_active = True  # Aktivoi puhekupla törmäyksen jälkeen
                    speech_bubble_text = cat["text"]  # Aseta puhekuplan teksti
                    speech_bubble_timer = 0  # Nollaa ajastin jokaisen uuden törmäyksen yhteydessä
            ...
            # Piirrä puhekupla, jos aktiivinen
            if speech_bubble_active:
                draw_speech_bubble(speech_bubble_text, (player_rect.x + cat_width // 2, player_rect.y))
                speech_bubble_timer += 1  # Lisää ajastinta
                if speech_bubble_timer > 120:  # Näytä 2 sekuntia 60 FPS:llä
            ...
            # Näytä pisteet ja taso
            score_text = font.render(f"Pisteet: {score}/{num_cats}", True, WHITE)
            window.blit(score_text, (10, 10))  # Aseta pisteiden teksti vasempaan yläkulmaan
